...
object Main extends  App {
  ...
  def wholeBoard: Vector[Square] = {
    Range.inclusive(1, 8).flatMap(x => {
      Range.inclusive(1, 8).map(y => {
        Square(x, y)
      })
    }).toVector
  }
  ...
  def canBePlaced(queenToPlace: Square, availableSquares: Vector[Square]): Option[Vector[Square]] = {

    if(availableSquares.nonEmpty) {
      if (availableSquares.contains(queenToPlace))
        Some(availableSquares.toSet.diff(queensCheckedSquares(queenToPlace).toSet).toVector)
      else
        None
    } else {
      None
  ...
  def placeQueens(toPlace: Int, available: Vector[Square]): Option[PlacedQueens] = {

    if(toPlace <= 0) {
      Some(Vector.empty[Square])
    } else {
      val result = available.foldLeft(Vector.empty[Square])((prev, queenToPlace: Square) => {
  ...
  def printQueens(placedQueens: PlacedQueens): Unit = {
    ...
    Range.inclusive(1, 8).foreach(x => {
      Range.inclusive(1, 8).foreach(y => {
